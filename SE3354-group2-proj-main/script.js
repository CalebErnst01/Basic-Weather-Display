let weather = {
    ...
    displayHistory: function(data){
        ...
        if(displayCelsius) //if displayCelsius is TRUE
        {
            document.querySelector(".hist_temp").innerText = "Temperature: " + ( (temperature - 30) /2 ) + "°C";
            document.querySelector(".hist_temp_min").innerText = "Low: " + ( (low - 30) /2) + "°C";
            document.querySelector(".hist_temp_max").innerText = "High: " + ( (high - 30) /2) + "°C";
            document.querySelector(".hist_feels_like").innerText = "Felt like: " + ( (feel - 30) /2) + "°C";
        }
        else //fahrenheit selected by default
        {
    ...
    display7: function(data){
        var one = data.list[0].temp.max; //can hard code the first 7 days
        ...
        let months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; //will display the month and day
        ...
        if(displayCelsius)
        {
            document.querySelector(".one").innerText = time + ": " + ( (one - 30) /2) + "°C";
            document.querySelector(".two").innerText = time1 + ": " + ( (two - 30) /2) + "°C";
            document.querySelector(".three").innerText = time2 + ": " + ( (three - 30) /2) + "°C";
            document.querySelector(".four").innerText = time3 + ": " + ( (four - 30) /2) + "°C";
            document.querySelector(".five").innerText = time4 + ": " + ( (five - 30) /2) + "°C";
            document.querySelector(".six").innerText = time5 + ": " + ( (six - 30) /2) + "°C";
            document.querySelector(".seven").innerText = time6 + ": " + ( (seven -30) /2) + "°C";
        }
        else
        {
    ...
    display24: function(data){
        for(let i = 1; i <= data.list.length; i++){ //for loop to run through the list and get the first 24 hours
    ...
    fetch24: function(city){
        fetch( //uses a hourly api version
    ...
    displayWeather: function(data){
        ...
        if(displayCelsius)
        {
            document.querySelector(".temp").innerText = ( (temp - 30) /2) + " °C";

            document.querySelector(".temp_min").innerText = "Lowest: " + ( (temp_min - 30) /2) + " °C";
            document.querySelector(".temp_max").innerText = "Highest: " + ( (temp_max - 30) /2) + " °C";
            document.querySelector(".feels_like").innerText = "Feels like: " + ( (feels_like - 30) /2) + " °C";
            document.querySelector(".humidity").innerText = "Humidity: " + ( (humidity - 30) /2) + " %";

            document.querySelector(".pressure").innerText = "Pressure: " + pressure + " hpa";
            document.querySelector(".wind").innerText = "Wind Speed: " + (speed * 1.609) + " kph";
        }
        else
        {
        ...
        if(displayCelsius)
            ...
            document.querySelector(".visibility").innerText = "Visibility: " + (visibility * 3.281) + " ft";
